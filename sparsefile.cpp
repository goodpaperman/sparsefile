...
int main(int argc, char* argv[])
{
    if (argc < 3)
    {
        std::cout << "Usage: sparsefile file length(in GB) [set-end-of-file] [set-file-valid-data] [write-whole-file] [fill-char]\n";
        return 1; 
    }
    ...
    if (argc > 3)
        USE_SET_END_OF_FILE = atoi(argv[3]);

    if (argc > 4)
        USE_SET_FILE_VALID_DATA = atoi(argv[4]);

    if (argc > 5)
        WRITE_WHOLE_FILE = atoi(argv[5]);
